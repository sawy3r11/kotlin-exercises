    ...
    operator fun minus(other: Money)=
        when {
            this.currency != other.currency -> {
                throw IllegalArgumentException("Cannot subtract money with different currencies: ${this.currency} and ${other.currency}")
            }
            else -> Money(this.amount - other.amount, this.currency)
        }

    operator fun unaryMinus() = Money(-this.amount, this.currency)

    operator fun times(factor: Int) = Money(this.amount * BigDecimal(factor), this.currency)
}

operator fun Money.plus(other: Money): Money =
    when {
        this.currency != other.currency -> {
            throw IllegalArgumentException("Cannot add money with different currencies: ${this.currency} and ${other.currency}")
        }
