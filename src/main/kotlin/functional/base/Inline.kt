...
fun main() {
    val list: List<Any> = listOf(1, "A", 3, "B")
...
inline fun <reified T> List<Any>.anyOf(): Boolean = this.any { it is T }
inline fun <reified T> List<Any>.firstOfOrNull(): T? = this.firstOrNull() { it is T } as T?
inline fun <reified A, reified B> Map<*, *>.filterValuesInstanceOf(): Map<A, B> =
    this.entries
        .filter { it.key is A && it.value is B }
